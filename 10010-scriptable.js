 ...
 * 中国联通信息展示和自动签到
 * 
 * ua 一定要修改成自己的  这也就是为什么你的cookie没几天就过期，以及为什么不能打开联通app的原因
 * ua对   那么app和scriptable脚本同时打开都没事
 * 
 *
 */
...
const get_mlData = async () => {
  const cachePath = files.joinPath(files.documentsDirectory(), 'ml-test');  
  const headers = {
    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 unicom{version:iphone_c@9.0201}'
  };
...
const getData = async () => {
  const cachePath = files.joinPath(files.documentsDirectory(), 'Chinatest');
  ...
  const url = 'http://192.168.1.7/data';
  const req = new Request(url);
  req.headers = {
    'User-Agent': 'ChinaUnicom.x CFNetwork iOS/16.1 unicom{version:iphone_c@9.0201}',
    cookie: clientCookie
  };
  try {
    const data = await req.loadJSON();
